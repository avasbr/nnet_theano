...
hyperspace = {'ae_params':[
				  	{'l1_reg':hp.choice('l1_lambda',[None,hp.loguniform('l1_decay',log(1e-5),log(10))])},
			  		{'l2_reg':hp.choice('l2_lambda',[None,hp.loguniform('l2_decay',log(1e-5),log(10))])},
			  		{'beta':hp.loguniform('beta',log(1e-2),log(10))},
			  		{'sparse_rho':hp.loguniform('sparse_rho',log(1e-2),log(1))}
				],
			  'optim_params':[
			  	...
			  	{'init_method':hp.choice('init',['gauss','fan-io'])},
...
# sparse autoencoder parameters
ae_params = {'d':d,'num_hids':[25],'activs':['sigmoid','sigmoid'],
'loss_terms':['squared_error','l2_reg','sparsity'],'l2_decay':0.0004294577310202803,
'beta':0.5445760025914531,'rho':0.01623096175544125}
...
# {'beta': 0.5445760025914531, 'd': 64, 'rho': 0.01623096175544125, 'num_hids': [25], 
# 'l1_decay': None, 'loss_terms': ['squared_error', 'sparsity', 'l2_reg'], 'l2_decay': 0.0004294577310202803, 
# 'activs': ['sigmoid', 'sigmoid']}

# {'learn_rate': 0.7477276197510565, 'scale_factor': 0.08740630845952124, 'optim_type': 'minibatch', 
# 'batch_size': 243, 'optim_method': 'RMSPROP', 'init_method': 'fan-io', 'rho': 0.11547856267104184, 'num_epochs': 80}
