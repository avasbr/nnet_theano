...
k = max(train_lbl)+1
y_oh = np.zeros((m,k)) # 'one-hot' representation
...
y_tr = nu.floatX(y_oh[pidx[:m_tr],:])

X_val = nu.floatX(X[pidx[m_tr:],:])
y_val = nu.floatX(y_oh[pidx[m_tr:],:])
...
y_te = nu.floatX(idx2numpy.convert_from_file(test_lbl_path)) 

print 'Training...'

mln_params = {'d':d,'k':k,'n_hid':[],'activ':[nu.softmax],'cost_type':'cross_entropy','L2_decay':0.001}
optim_params = {'method':'SGD','n_iter':1000,'learn_rate':0.1}
nnet = mln.MultilayerNet(**mln_params)
nnet.fit(X_tr,y_tr,**optim_params)

pred,mce = nnet.get_predict_fns()

print 'Performance on test set:'
print 'Accuracy:',100.*(1-mce(X_te,y_te)),'%'